...
import { Trophy, Target, Star, Flame, TrendingUp, Calendar, Medal, Crown, Zap } from 'lucide-react'
...
const getRarityStatsColor = (rarity: string) => {
  const colorMap = {
  ...
  return colorMap[rarity as keyof typeof colorMap] || 'text-gray-400'
}
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-white">
                {stats.recentlyEarned.length > 0 
                  ? (stats.recentlyEarned.reduce((sum, a) => sum + (a.difficulty || 1), 0) / stats.recentlyEarned.length).toFixed(1)
                  : '0.0'
                }
          ...
          <div className="flex gap-1">
            {Array.from({ length: 5 }, (_, i) => {
              const avgDifficulty = stats.recentlyEarned.length > 0 
                ? stats.recentlyEarned.reduce((sum, a) => sum + (a.difficulty || 1), 0) / stats.recentlyEarned.length
                : 0
              return (
                <Star
                  key={i}
                  size={12}
                  fill={i < Math.round(avgDifficulty) ? 'currentColor' : 'none'}
                  className={i < Math.round(avgDifficulty) ? 'text-yellow-400' : 'text-slate-600'}
                />
              )
            })}
          </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              {Object.entries(stats.byRarity).map(([rarity, data]) => {
                ...
                const rarityColor = getRarityStatsColor(rarity)
