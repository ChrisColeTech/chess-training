...
const getRarityFilterColor = (rarity: string) => {
  const colorMap = {
    Common: 'bg-slate-600 text-slate-200 border-slate-500',
    Rare: 'bg-blue-600 text-blue-100 border-blue-500',
    Epic: 'bg-purple-600 text-purple-100 border-purple-500', 
    Legendary: 'bg-orange-600 text-orange-100 border-orange-500',
    Mythic: 'bg-pink-600 text-pink-100 border-pink-500'
  }
  return colorMap[rarity as keyof typeof colorMap] || colorMap.Common
}

const availableAchievementCategories = [
  'Tactical', 'Rating', 'Study', 'Special', 'Endgame', 'Opening', 'Puzzles', 'Games', 'Streaks', 'Milestones'
] as const

const availableAchievementRarities = [
...
const achievementSortOptions = [
  ...
  { key: 'earnedAt', label: 'Date Earned' },
  { key: 'difficulty', label: 'Difficulty' }
]

const defaultAchievementFilters = {
  categories: [],
  rarities: [],
  statuses: [],
  earnedOnly: false,
...
export const AchievementFilters: React.FC<AchievementFiltersProps> = ({
  filters,
  onFiltersChange,
  availableCategories = availableAchievementCategories,
  availableRarities = availableAchievementRarities,
  theme,
  isCollapsed = false
}) => {
  ...
  const clearAllFilters = () => {
    onFiltersChange(defaultAchievementFilters)
  }
        ...
        {showAdvanced && (
          <div className="mt-4 space-y-4 pt-4 border-t border-slate-700/50">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {achievementSortOptions.map(({ key, label }) => (
                  <button
                    key={key}
