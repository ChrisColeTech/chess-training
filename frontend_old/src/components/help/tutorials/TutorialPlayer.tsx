...
import { useTutorials } from '@/hooks/tutorials/useTutorials'
...
const RelatedTutorials: React.FC<RelatedTutorialsProps> = ({ tutorial }) => {
  // Use API hook for related tutorials
  const { useTutorialList } = useTutorials()
  const { data: allTutorials, isLoading } = useTutorialList(tutorial.category)
  
  // Filter related tutorials based on same category, excluding current tutorial
  const relatedTutorials = React.useMemo(() => {
  ...
  }, [allTutorials, tutorial.id, tutorial.category])
        ...
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className="flex gap-3 animate-pulse">
              <div className="w-16 h-12 bg-slate-700/50 rounded"></div>
