...
const hashSizeOptions = [128, 512, 1024]
const depthRange = {
  min: 5,
  max: 30,
  step: 1,
  labels: { min: 'Fast', max: 'Deep' }
}
const multiPVConfig = { min: 1, max: 5 }
const contemptRange = { min: -100, max: 100, step: 10, default: 0 }
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex justify-between text-xs opacity-60 mt-1">
                <span>{depthRange.labels.min}</span>
                <span>Balanced ({localDepth[0]})</span>
                <span>{depthRange.labels.max}</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center justify-center gap-3">
                <Button 
                  ...
                  disabled={localMultiPV <= multiPVConfig.min}
                ...
                <Button 
                  ...
                  variant="outline" 
                  onClick={() => handleMultiPVChange(1)}
                  disabled={localMultiPV >= multiPVConfig.max}
                  className="w-8 h-8 p-0"
                >
        ...
        <TabsContent value="setup" className="mt-6 space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Slider
                value={[analysisSettings.contempt || contemptRange.default]}
                onValueChange={([value]) => onSettingsChange({ contempt: value })}
                max={contemptRange.max}
                min={contemptRange.min}
                step={contemptRange.step}
                className="w-full"
              />
              <div className="text-xs opacity-60 mt-1 text-center">
                Current: {analysisSettings.contempt || contemptRange.default}
              </div>
            </div>
          </div>
        </TabsContent>
