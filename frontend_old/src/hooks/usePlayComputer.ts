...
export const usePlayComputer = (): PlayComputerHookReturn => {
  ...
  // Local constants instead of importing from @/data
  const timeControlConfigs = [
    { type: 'Blitz', minutes: 3, increment: 0, description: '3 minutes' },
    { type: 'Rapid', minutes: 10, increment: 0, description: '10 minutes' },
    { type: 'Classical', minutes: 30, increment: 0, description: '30 minutes' },
    { type: 'Unlimited', minutes: 0, increment: 0, description: 'No time limit' }
  ] as const
  
  const defaultGameSetup: GameSetup = {
    opponent: null,
    playerColor: 'white',
    timeControl: timeControlConfigs[0],
  ...
  const GAME_TIMER_CONFIG = {
    INTERVAL_MS: 1000,
    TIME_FORFEIT_THRESHOLD: 0
  }
  
  const MOVE_TIMING = {
    AI_MOVE_DELAY: 1000,
    PLAYER_MOVE_DELAY: 300
  }
  ...
  useEffect(() => {
    ...
    // Mock available opponents - would come from API
    const opponents: AIOpponent[] = [
      {
        ...
        isUnlocked: true,
        avatar: '/avatars/training-bot.png',
        strengths: ['Patient play'],
        weaknesses: ['Tactical oversight'],
  ...
  const startGameTimer = useCallback(() => {
    ...
    gameTimerRef.current = setInterval(() => {
      setGameState(prevState => {
        ...
        timeRemaining[currentPlayer] -= GAME_TIMER_CONFIG.INTERVAL_MS

        // Check for time forfeit
        if (timeRemaining[currentPlayer] <= GAME_TIMER_CONFIG.TIME_FORFEIT_THRESHOLD) {
  ...
  const makeMove = useCallback(async (from: string, to: string): Promise<boolean> => {
    ...
    if (result.updatedState.status === 'active') {
      ...
      }, MOVE_TIMING.PLAYER_MOVE_DELAY) // Small delay for better UX
    } else {
      stopGameTimer()
