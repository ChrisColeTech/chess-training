...
// Mock data removed - now using real API data from useAuth and useProfile

/**
 * Create a default account structure for initialization
 */
const createDefaultAccount = (profileData?: any): UserAccount => ({
  accountId: 'user_001',
  ...
  profile: profileData || {
    ...
    avatarUrl: '',
    ratings: { rapid: 0, blitz: 0, bullet: 0, classical: 0, puzzle: 0 },
    fideRating: null,
    timezone: 'UTC',
    language: 'en-US',
    title: null,
  ...
  security: {
    twoFactorAuth: { enabled: false, method: 'none', backupCodes: [], enabledAt: null },
    passwordRequirements: { lastChanged: Date.now(), requiresChange: false, strength: 'medium' },
    loginNotifications: { newDevice: true, suspiciousActivity: true, failedAttempts: true },
    sessionSettings: { maxSessions: 5, sessionTimeout: 24 * 60 * 60 * 1000, requireReauth: false },
    apiAccess: { enabled: false, rateLimit: 100, allowedOrigins: [] }
  },
  subscription: {
    tier: 'free',
    status: 'active',
    billingCycle: 'monthly',
    currentPeriod: { start: Date.now(), end: Date.now() + (30 * 24 * 60 * 60 * 1000) },
    features: {},
    payment: { method: null, lastFour: null, expiresAt: null, autoRenew: false },
    billingHistory: [],
    usage: { puzzlesSolved: 0, gamesPlayed: 0, studyHours: 0, coachingSessions: 0 }
  },
  connectedServices: [],
  privacy: {
    profileVisibility: 'public',
    ...
    dataCollection: { analytics: true, marketing: false, personalization: true, research: false },
    communications: { email: true, push: true, sms: false, newsletter: false },
    dataRetention: { deleteInactive: false, retentionPeriod: 365, exportBeforeDelete: true }
  },
  badges: {
    verification: { emailVerified: false, phoneVerified: false, identityVerified: false, addressVerified: false },
    trust: { accountAge: 0, gamesPlayed: 0, communityRating: 0, reportCount: 0 },
    achievements: [],
    certifications: { chessCoach: false, tournament: false, streamer: false, moderator: false }
  },
  loginHistory: [],
  dataExports: []
})
...
export const useAccount = (): AccountHookReturn => {
  // API hooks
  const { useUserProfile, updateProfile, isUpdatingProfile, updateProfileError } = useProfile()
  ...
  // State for complex operations not yet in API
  ...
  /**
   * Update user profile - now uses API
   */
  const updateProfileHandler = useCallback(async (updates: Partial<UserProfile>): Promise<boolean> => {
    ...
    try {
      // Use the API-based profile update
      await updateProfile(updates)
      soundFX.playSuccess()
      return true
    } catch (err: any) {
      const errorMsg = err.message || updateProfileError?.message || 'Failed to update profile'
      setError(errorMsg)
      soundFX.playError()
      return false
    } finally {
  ...
  /**
   * Change user password - now uses API
   */
  const changePasswordHandler = useCallback(async (currentPassword: string, newPassword: string): Promise<boolean> => {
    if (operationInProgress.current) return false
    ...
    try {
      // Use the API-based password change
      await changePassword({ currentPassword, newPassword })
      
      // Update local account state for password change timestamp
      setAccount(prev => prev ? {
        ...prev,
        security: {
          ...prev.security,
      ...
      const errorMsg = err.message || changePasswordError?.message || 'Password change failed'
      setError(errorMsg)
      soundFX.playError()
      return false
    } finally {
  ...
  return {
    // State (merge API and local state)
    account: account || (profileData ? createDefaultAccount(profileData) : null),
    isLoading: isLoading || isProfileLoading || isUpdatingProfile || isChangingPassword,
