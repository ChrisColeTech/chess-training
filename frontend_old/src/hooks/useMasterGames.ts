...
export const useMasterGames = (): MasterGamesHookReturn => {
  ...
  // Game data state
  const [filteredGames, setFilteredGames] = useState<MasterGame[]>([])
  const [selectedGame, setSelectedGame] = useState<MasterGame | null>(null)
  const [currentMove, setCurrentMoveState] = useState(0)
  const [libraryStatsData] = useState<LibraryStats>({
    totalGames: 0,
    totalPlayers: 0,
    dateRange: { earliest: '', latest: '' },
    ratingRange: { min: 2000, max: 2800 },
    popularOpenings: [],
    recentlyAdded: 0
  })
  ...
  useEffect(() => {
    ...
    if (filters.searchTerm) {
      filtered = games.filter(game => 
        game.white.name.toLowerCase().includes(filters.searchTerm.toLowerCase()) ||
        game.black.name.toLowerCase().includes(filters.searchTerm.toLowerCase()) ||
        game.opening.name.toLowerCase().includes(filters.searchTerm.toLowerCase())
      )
    }
