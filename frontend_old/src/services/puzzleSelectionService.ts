...
export class PuzzleSelectionService {
  ...
  static getRecommendedCategory(categories: PuzzleCategory[]): PuzzleCategory | null {
    ...
    return activeCategories.reduce((lowest, current) => {
      const currentProgress = getCategoryProgress(current.id)
      const lowestProgress = getCategoryProgress(lowest.id)
      return currentProgress.solved / currentProgress.total < lowestProgress.solved / lowestProgress.total ? current : lowest
    })
  }
  ...
  static filterCategories(categories: PuzzleCategory[], filters: PuzzleSelectionFilters): PuzzleCategory[] {
    ...
    if (!filters.showCompleted) {
      filtered = filtered.filter(cat => {
        ...
        return (progress.solved / progress.total) * 100 < 100
      })
    }
    ...
    switch (filters.sortBy) {
      ...
      case 'progress':
        filtered.sort((a, b) => {
          const aProgress = getCategoryProgress(a.id)
          const bProgress = getCategoryProgress(b.id)
          const aPercent = (aProgress.solved / aProgress.total) * 100
          const bPercent = (bProgress.solved / bProgress.total) * 100
