...
export const useGameReview = (): GameReviewHookReturn => {
  ...
  const [availableGames, setAvailableGames] = useState<GameReview[]>(mockGameReviews)
  const [gameCollections, setGameCollections] = useState<GameCollection[]>(mockGameCollections)
  ...
  useEffect(() => {
    ...
    const recentGames = getRecentGames(1)
    if (recentGames.length > 0 && !currentGame) {
      selectGame(recentGames[0].id)
    }
  }, [currentGame])
