...
import { mockEndgamePositions, mockEndgameCategories, mockCompositions } from '@/data/endgamePositions'
import type {
  EndgamePosition,
  EndgameCategoryInfo,
  EndgameCategory,
...
export const useEndgameLibrary = (): EndgameLibraryHookReturn => {
  // Data state
  const [categories] = useState<EndgameCategoryInfo[]>(mockEndgameCategories)
  const [positions] = useState<EndgamePosition[]>(mockEndgamePositions)
  const [compositions] = useState<EndgameComposition[]>(mockCompositions)
  ...
  const [isLoadingPositions, _setIsLoadingPositions] = useState(false)
  const [error, setError] = useState<string | null>(null)

  /**
   * Initialize study progress for all positions
   */
  useEffect(() => {
    const initialProgress: Record<string, StudyProgress> = {}
    
    positions.forEach(position => {
      // Generate some mock progress data for demonstration
      const progress = EndgameLibraryService.generateInitialProgress(position.id)
      
      // Add some mock data for certain positions
      if (position.id === 'kpk_basic') {
        progress.sessionsCompleted = 3
        progress.totalStudyTime = 45
        progress.practiceStats.play_engine.attempts = 5
        progress.practiceStats.play_engine.successes = 4
        progress.variationsMastered = ['main_line', 'defense_var1']
        progress.masteryLevel = EndgameLibraryService.calculateMastery(progress)
        progress.lastStudied = Date.now() - 86400000 // 1 day ago
      } else if (position.id === 'lucena_position') {
        progress.sessionsCompleted = 7
        progress.totalStudyTime = 120
        progress.practiceStats.guided_study.attempts = 8
        progress.practiceStats.guided_study.successes = 7
        progress.variationsMastered = ['bridge_method', 'cut_off_king', 'rook_4th']
        progress.masteryLevel = EndgameLibraryService.calculateMastery(progress)
        progress.lastStudied = Date.now() - 172800000 // 2 days ago
        progress.isBookmarked = true
      }

      initialProgress[position.id] = progress
    })
  ...
  const analyzePosition = useCallback(async (position: EndgamePosition) => {
    ...
    try {
      const analysis = await EndgameLibraryService.analyzePosition(position)
  ...
  const queryTablebase = useCallback(async (fen: string): Promise<TablebaseQueryResult> => {
    ...
    try {
      const result = await EndgameLibraryService.queryTablebase(fen)
   ...
   */
  const startPracticeSession = useCallback((position: EndgamePosition, type: PracticeType) => {
    const session = EndgameLibraryService.createPracticeSession(position, type)
  ...
  const endPracticeSession = useCallback(() => {
    ...
    // Update progress
    setStudyProgress(prev => {
      const currentProgress = prev[positionId] || EndgameLibraryService.generateInitialProgress(positionId)
      const updatedProgress = {
        ...currentProgress,
        sessionsCompleted: currentProgress.sessionsCompleted + 1,
        totalStudyTime: currentProgress.totalStudyTime + sessionTime
      }
      
      updatedProgress.masteryLevel = EndgameLibraryService.calculateMastery(updatedProgress)
   ...
   */
  const updateProgress = useCallback((positionId: string, progressUpdate: Partial<StudyProgress>) => {
    setStudyProgress(prev => {
      const currentProgress = prev[positionId] || EndgameLibraryService.generateInitialProgress(positionId)
      const updatedProgress = { ...currentProgress, ...progressUpdate }
      
      // Recalculate mastery if relevant fields changed
      if ('practiceStats' in progressUpdate || 'variationsMastered' in progressUpdate) {
        updatedProgress.masteryLevel = EndgameLibraryService.calculateMastery(updatedProgress)
      }
