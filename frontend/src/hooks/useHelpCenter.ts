...
export const useHelpCenter = (): HelpCenterHookReturn => {
  ...
  const [tutorialList] = useState<Tutorial[]>(tutorials)
  const [categories] = useState<CategoryInfo[]>(helpCategories)
  const [config] = useState<HelpCenterConfig>(helpCenterConfig)
  ...
  const performSearch = useCallback(async (query: string, filters?: Partial<SearchFilters>): Promise<void> => {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        const results = performMockSearch(query, currentFilters)
