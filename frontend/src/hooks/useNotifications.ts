...
import { mockNotificationSettings, mockNotifications, mockNotificationStats } from '@/data/notificationSettings'
import type {
...
export const useNotifications = (): NotificationsHookReturn => {
  ...
  // Notifications state
  const [notifications, setNotifications] = useState<NotificationInstance[]>(mockNotifications)
  const [notificationStats, _setNotificationStats] = useState<NotificationStats>(mockNotificationStats)
  const [error, setError] = useState<string | null>(null)

  // Settings change tracking
  const [originalSettings, setOriginalSettings] = useState<NotificationSettings>(mockNotificationSettings)
