...
import {
  searchOpenings,
  getVariationsForPosition,
  getMasterGamesForOpening,
  analyzePosition as analyzePositionService,
  defaultOpeningFilters,
  mockOpenings
} from '@/data/openingsDatabase'
import type {
  ChessOpening,
  MoveVariation,
  MasterGame,
...
export const useOpeningExplorer = (): OpeningExplorerHookReturn => {
  ...
  // Current state
  const [currentOpening, setCurrentOpening] = useState<ChessOpening | null>(mockOpenings[0])
  ...
  useEffect(() => {
    const initializeExplorer = async () => {
      try {
        // Load variations for starting position
        const variations = await getVariationsForPosition(currentPosition)
        setCurrentVariations(variations)
        
        // Load master games for current opening
        if (currentOpening) {
          setIsLoadingGames(true)
          const games = await getMasterGamesForOpening(currentOpening.eco)
  ...
  const selectOpening = useCallback(async (opening: ChessOpening) => {
    try {
      ...
      // Load variations for this position
      const variations = await getVariationsForPosition(opening.fen)
      setCurrentVariations(variations)
      
      // Load master games
      setIsLoadingGames(true)
      const games = await getMasterGamesForOpening(opening.eco)
      setMasterGames(games)
      setIsLoadingGames(false)
  ...
  const makeMove = useCallback((from: string, to: string): boolean => {
    try {
      ...
      // Load variations for new position (async)
      getVariationsForPosition(newPosition).then(setCurrentVariations)
      
      // Clear current opening if we've moved away from known openings
      const matchingOpening = mockOpenings.find(opening => opening.fen === newPosition)
      if (matchingOpening && matchingOpening.eco !== currentOpening?.eco) {
        setCurrentOpening(matchingOpening)
      } else if (!matchingOpening) {
  ...
  const resetPosition = useCallback(async () => {
    try {
      ...
      // Load starting position variations
      const variations = await getVariationsForPosition(chess.fen())
      setCurrentVariations(variations)
  ...
  const loadPosition = useCallback(async (fen: string, moves: string[]) => {
    try {
      ...
      const matchingOpening = mockOpenings.find(opening => opening.fen === fen)
      setCurrentOpening(matchingOpening || null)
      
      // Load variations
      const variations = await getVariationsForPosition(fen)
      setCurrentVariations(variations)
  ...
  const searchOpeningsHandler = useCallback(async (query: string = '') => {
    try {
      ...
      const results = await searchOpenings(searchFilters)
  ...
  const analyzePosition = useCallback(async () => {
    try {
      ...
      const analysis = await analyzePositionService(currentPosition)
  ...
  const selectVariation = useCallback(async (variation: MoveVariation) => {
    try {
      ...
      // Load variations for new position
      const variations = await getVariationsForPosition(newPosition)
      setCurrentVariations(variations)
