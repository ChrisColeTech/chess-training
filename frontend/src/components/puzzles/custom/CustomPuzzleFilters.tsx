...
export const CustomPuzzleFilters: React.FC<CustomPuzzleFiltersComponentProps> = ({
  ...
  const sources: PuzzleSource[] = ['user-created', 'imported', 'community', 'lichess', 'chess-com']
            ...
            <div className="flex flex-wrap gap-2">
              {sources.map(source => {
                ...
                const getSourceLabel = (s: string) => {
                  switch (s) {
                    case 'user-created': return '✍️ User Created'
                    case 'imported': return '📥 Imported'
                    case 'community': return '👥 Community'
                    case 'lichess': return '♜ Lichess'
                    case 'chess-com': return '<FaChessKing className="w-4 h-4 inline" /> Chess.com'
                    default: return s
                  }
                }
