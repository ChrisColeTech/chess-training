...
export const CustomPuzzleInfo: React.FC<CustomPuzzleInfoProps> = ({
  ...
  const getSourceIcon = (source: string) => {
    switch (source) {
      case 'user-created': return '✍️'
      case 'imported': return '📥'
      case 'community': return '👥'
      case 'lichess': return '♜'
      case 'chess-com': return '<FaChessKing className="w-4 h-4 inline" />'
      default: return '<Target className="w-4 h-4 inline" />'
    }
  }
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <div className="flex items-center text-gray-400">
              <span className="mr-1">{getSourceIcon(puzzle.source)}</span>
